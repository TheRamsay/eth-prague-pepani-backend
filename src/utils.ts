...
export async function getVotingPower(voterAddress: string, spaceId: number, blockHeight: bigint | string): Promise<bigint> {
    const res = await backendActor.get_all_evm_strategies_by_space_id({ id: spaceId }) as QueryResponse;
    const strategies = parseJsonToModel<GetEvmStrategy[]>(res);

    let power = 0n;

    for (const strategy of strategies) {
        const tx = await encodeTransaction(strategy.contractAddress, strategy.configString.replace("$voterAddress", voterAddress.replace("0x", "")), 1);
...
export async function triggerEvent(spaceId: number, eventType: number, eventData: object) {
    const events = parseJsonToModel<SpaceEvent[]>(await backendActor.get_all_space_events_by_space_id({ id: spaceId }) as QueryResponse);
...
export async function encodeTransaction(to: string, data: string, chainId: number) {
